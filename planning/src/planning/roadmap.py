...
class Roadmap(object):
    ...
    def connect_vertices(self, vertices):
        ...
        for i in range(self.num_vertices):
            ...
            h[i, i + 1 :] = self.problem.compute_distance(u, vertices[i + 1 :, :])

            if self.directed:
                h[i, :i] = self.problem.compute_distance(u, vertices[:i, :])
    ...
    def add_node(self, state, is_start):
        ...
        if is_start:
            self.start = index
            h = self.problem.compute_distance(state, self.vertices)
        else:
            self.goal = index
            h = self.problem.compute_distance(self.vertices, state)
        h[h > self.connection_radius] = 0
        (i,) = h.nonzero()
        i = i.reshape((-1, 1))
    ...
    def compute_path_length(self, vpath):
        ...
        return self.problem.compute_distance(q[:-1, :], q[1:, :]).sum()
